...
sendbullet proc near                                          ; 
	...
	mov dx , 3FDH		; Line Status Register
    ags:  In al , dx 			;Read Line Status
  	test al , 00100000b
    JZ ags            ;Not empty

      ;If empty put the VALUE in Transmit data register
        mov bx,player1bulletPosX
        mov al,bh
        mov ah,0
    ...
    ;><><><><><><<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>
    mov dx , 3FDH		; Line Status Register
    AG2:  In al , dx 			;Read Line Status
  	test al , 00100000b
    JZ AG2            ;Not empty

      ;If empty put the VALUE in Transmit data register
  		mov dx , 3F8H		; Transmit data register
  		mov ax , player1bulletPosY
  		out dx , ax
	...
	mov dx , 3FDH		; Line Status Register
    ags2:  In al , dx 			;Read Line Status
  	test al , 00100000b
    JZ ags2            ;Not empty

      ;If empty put the VALUE in Transmit data register
        mov bx,player1bulletPosY
        mov al,bh
        mov ah,0
    ...
    mov dx , 3FDH		; Line Status Register
    ags3:  In al , dx 			;Read Line Status
  	test al , 00100000b
    JZ ags3            ;Not empty

    mov al , player1IsThereBullet
  	mov dx , 3F8H		; Transmit data register
  	out dx , al
